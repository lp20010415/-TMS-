...
<body>
	...
	<form name="form1" class="form" action="" method="post" id="form1" style="float:right;border:1px solid black;margin:2px;overflow:auto;white-space:nowarp;width:auto;height:490px;background:white;text-align:center">
				<nobr><a>夹具编号</a><a>名称</a><a>Family</a><a>Model</a><a>PartNo</a><a>生产日期</a><a>建档日期</a><a>采购单据号</a><a>经办人</a><a>库位</a><a>照片</a><a>状态</a><a>所借人</a><a>归还人</a><a>借出时间</a><a style="border-right:1px solid black;">操作</a></nobr>
	    		<%
	    		int i = 0;
	    		int k = 0;
	    		while(rs.next()){ 
	    		...
	    		<nobr><a ><%=rs.getString("夹具编号") %></a><a><%=rs.getString("名称") %></a><a><%=rs.getString("Family") %></a><a><%=rs.getString("Model") %></a><a><%=rs.getString("PartNo") %></a><a><%=rs.getString("生产日期") %></a><a><%=rs.getString("建档日期") %></a><a><%=rs.getString("采购单据号") %></a><a><%=rs.getString("经办人") %></a><a><%=rs.getString("库位") %></a><a><img style="width:79px;height:45px;margin:0;" src="<%=rs.getString("照片") %>"></a><a><%=rs.getString("状态") %></a><a><%=rs.getString("所借人或上报人")%></a><a><input style="border:0;width:78px;;height:30px;" name="givebackman<%=i %>" value=""/></a><a><%=rs.getString("借出时间") %></a><a style="border-right:1px solid black;"><button onclick="btn维修<%=i %>()" type="button" id="btn" style="width:30px;height:25px;">维修</button>&nbsp;<button onclick="btn归还<%=i %>()" type="button" id="btn" style="width:30px;height:25px;">归还</button></a></nobr>
