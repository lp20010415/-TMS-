...
<!--
夹具情况		
	上报夹具-data_jiaju_repair,状态暂停,两个选项(维修完成->data_jiaju_repaired(记录,维修完成时间,维修员);报废->data_jiaju_bad_1->data_jiaju_all_bad_1(记录上报时间,上报人));-this
	已维修夹具-data_jiaju_repaired,(入库)->data_jiaju_all_repaired(记录,维修完成时间,入库时间);
	已上报的报废夹具-data_jiaju_bad_1(记录上报时间,上报人)
采购入库夹具
	申请采购入库夹具(填写信息)->data_jiaju_applied_1(记录申请时间,申请人)
	已申请的夹具-data_jiaju_applied_1
  ...
  <div class="layui-main">
    ...
    <ul class="layui-nav " >
      <li class="layui-nav-item layui-this">
        <a href="work-2-1.jsp" style="background-color: #f9e9c3;"><font color="black">操作界面</font></a> 
      </li>
		...
		<ul class="layui-nav layui-nav-tree layui-bg-blue" lay-filter="test" style="float:left;width:auto">
		  <li class="layui-nav-item layui-nav-itemed" >
		    ...
		    <dl style="background-color:#fff;" class="layui-nav-child layui-bg-blue">
		      <dd><a href="work-2-1.jsp" class="layui-this">上报的夹具</a></dd>
...
<div style="border:1px solid black;width:auto;float:right;margin:3px;margin-right:20px;">
	...
	<form name="form1" class="form" action="" method="post" id="form1" style="float:right;border:1px solid black;margin:2px;overflow:auto;white-space:nowarp;width:1020px;height:490px;background:white;">
				<nobr><a >夹具编号</a><a>名称</a><a>Family</a><a>Model</a><a>PartNo</a><a>生产日期</a><a>建档日期</a><a>采购单据号</a><a>经办人</a><a>库位</a><a>照片</a><a>状态</a><a style="border-right:1px solid black;">所借人/上报人</a><a>上报时间</a><a style="width:200px;">故障描述</a><a>故障图片</a><a style="width:200px">报废原因</a><a>维修员</a><a style="width:100px;">操作</a></nobr>
	    		<%
	    		int i = 0;
	    		int k = 0;
	    		while(rs.next()){ 
	    			i++;
	    		%>
	    		<nobr><a><input style="border:0;width:78px;;height:30px;" readonly="true" name="bianhao<%=i%>" value="<%=rs.getString("夹具编号") %>"></a><a><%=rs.getString("名称") %></a><a><%=rs.getString("Family") %></a><a><%=rs.getString("Model") %></a><a><%=rs.getString("PartNo") %></a><a><%=rs.getString("生产日期") %></a><a><%=rs.getString("建档日期") %></a><a><%=rs.getString("采购单据号") %></a><a><%=rs.getString("经办人") %></a><a><%=rs.getString("库位") %></a><a><img style="width:79px;height:45px;margin:0;" src="<%=rs.getString("照片") %>"></a><a><%=rs.getString("状态") %></a><a><%=rs.getString("所借人或上报人")%></a><a><%=rs.getString("上报时间") %></a><a style="width:200px;"><%=rs.getString("故障描述") %></a><a><img style="width:79px;height:45px;margin:0;" src="<%=rs.getString("故障图片") %>"></a><a style="width:200px;"><input style="border:0;width:200px;;height:30px;" name="brokenreason<%=i %>" placeholder="如非报废，无需填"></a><a><input style="border:0;width:78px;;height:30px;" name="repairman<%=i%>"></a><a style="border-right:1px solid black;width:100px;"><button onclick="btn维修完成<%=i %>()" type="button" id="btn" style="width:55px;height:25px;">维修完成</button>&nbsp;<button onclick="btn报废<%=i %>()" type="button" id="btn" style="width:30px;height:25px;">报废</button></a></nobr>
...
while(k>=1){%>	
function btn维修完成<%=k%>(){
		document.form1.action="维修完成.jsp";
		document.form1.submit();
	}
function btn报废<%=k%>(){
	document.form1.action="报废.jsp";
	document.form1.submit();
	}
